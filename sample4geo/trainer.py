...
from sample4geo.loss.cal_loss import cal_kl_loss, cal_loss, cal_triplet_loss, cal_cda_loss
from sample4geo.loss.klloss import cal_dkd_loss, DKD
from sample4geo.loss.triplet_loss import TripletLoss
import torch.nn as nn
from sample4geo.loss.CDA import CDALoss
    ...
    for query, reference, ids, labels in bar:
        ...
        if scaler:
            with (autocast()):  # -- 使用混合精度
                ...
                if train_config.handcraft_model is not True:
                    ...
                    features_cda_1, features_cda_2 = output1[0], output2[0]  # -- for DSA loss
                ...
                if torch.cuda.device_count() > 1 and len(train_config.gpu_ids) > 1:
                    ...
                    loss_tri = cal_triplet_loss(features_tri_1, features_tri_2, labels, triplet)

                    #----------------------------------------------------------------------------------------------------------
                    # lossall = train_config.weight_infonce * loss + train_config.weight_cls * loss_cls + train_config.weight_tri * loss_tri
                    lossall = train_config.weight_infonce * loss + train_config.weight_cls * loss_cls
                    #----------------------------------------------------------------------------------------------------------
        ...
        if train_config.verbose:
            ...
            monitor = {
                ...
                "loss_tri": "{:.4f}".format(train_config.weight_tri * loss_tri.item()),
